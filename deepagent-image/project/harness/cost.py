...
``docs/milestones/complete/milestone1.md`` §2.3.
...
class ModelRates:
    ...
    local-device measurement method for self-hosted models — see docs/specs/energy.md.
    """
...
def estimate_energy_wh(usage: dict, rates: ModelRates | None) -> float | None:
    ...
    successor to this estimate — specced, not implemented (see docs/specs/energy.md).
    """
...
def measure_local_energy_wh(*_args, **_kwargs) -> float:
    ...
    Specification (docs/specs/energy.md): wrap the model call (``before_model`` /
    ...
    raise NotImplementedError(
        ...
        "see docs/specs/energy.md and ModelRates.energy_source"
    )
