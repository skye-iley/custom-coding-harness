...
# =============================================================================
# Milestone 6 T4 — the telemetry block in the PR body
# =============================================================================
#
# Invariant 13 is the load-bearing one: a telemetry failure must NEVER be the
# reason a PR does not open. Every case below therefore asserts exit 0 as well as
# the body content.

import json  # noqa: E402

_OPEN_PR_SCRIPT = _PROJECT_ROOT / "workflows" / "git-pr" / "open-pr.sh"
_HARDCODED_BODY = (
    "Automated PR generated by Deep Agents. "
    "**Manual review required. Auto-merge disabled.**"
)


def _run_open_pr(tmp_path, summary_text=None):
    """Run open-pr.sh against a fake `gh` that dumps the body it was handed.

    A fake gh rather than a restructured script: the body is built after the
    gh/GH_TOKEN guards, so anything that stubs those out would test a code path
    the real run never takes.
    """
    bindir = tmp_path / "bin"
    bindir.mkdir()
    ...
    if summary_text is not None:
        (state / "session.json").write_text(summary_text, encoding="utf-8")

    env = dict(os.environ)
    env["PATH"] = f"{bindir}{os.pathsep}{env['PATH']}"
    env["GH_TOKEN"] = "fake"
    env["DEEPAGENTS_WORKSPACE"] = str(workspace)
    env["DEEPAGENTS_STATE_DIR"] = str(state)
    env["PYTHONPATH"] = str(_PROJECT_ROOT)
    proc = subprocess.run(
        ["sh", str(_OPEN_PR_SCRIPT)], env=env, capture_output=True, text=True
    )
    body = body_out.read_text(encoding="utf-8") if body_out.exists() else None
    return proc, body


def _summary_json(**over):
    tm = _load("harness.telemetry")
    summary = tm.derive_session(
        [
            {
                "schema": 1, "run_id": "run-x", "thread_id": "th", "topic": None,
                "turn": 1, "ts": "2026-01-01T00:00:00Z",
                "provider": "ollama", "model": "gemma4",
                "input": 41022, "output": 5120, "cache_read": 0, "cache_write": 0,
                "cost_usd": None, "cost_provenance": None, "energy_wh": None,
                "unpriced_calls": 0, "estimated_calls": 0,
                "duration_ms": 412330, "model_ms": 210400, "tool_ms": 88300,
                "retry_sleep_ms": 61000, "paced_sleep_ms": 0, "hitl_wait_ms": 0,
                "model_calls": 3, "tool_calls": {"read_file": 18}, "tool_errors": 0,
                "failed": False, "retry_count": 0, "context_trimmed": False,
                "interrupts": 0,
            }
        ],
        run_id="run-x",
...
@pytest.mark.skipif(not (_HAS_SH and _HAS_PY3), reason="needs sh and python3")
def test_open_pr_body_is_unchanged_without_a_summary(tmp_path):
    """Invariant 13: no summary => the body is byte-identical to the hardcoded
    text this used to pass as --body, and the step still exits 0."""
    proc, body = _run_open_pr(tmp_path, summary_text=None)
    assert proc.returncode == 0, proc.stderr
    assert body == _HARDCODED_BODY + "\n"


@pytest.mark.skipif(not (_HAS_SH and _HAS_PY3), reason="needs sh and python3")
def test_open_pr_body_is_unchanged_on_malformed_json(tmp_path):
    proc, body = _run_open_pr(tmp_path, summary_text="{not json at all")
    assert proc.returncode == 0, proc.stderr
    assert body == _HARDCODED_BODY + "\n"


@pytest.mark.skipif(not (_HAS_SH and _HAS_PY3), reason="needs sh and python3")
def test_open_pr_body_is_unchanged_on_an_unknown_schema(tmp_path):
    proc, body = _run_open_pr(tmp_path, summary_text=json.dumps({"schema": 99}))
    assert proc.returncode == 0, proc.stderr
    assert body == _HARDCODED_BODY + "\n"


@pytest.mark.skipif(not (_HAS_SH and _HAS_PY3), reason="needs sh and python3")
def test_open_pr_appends_the_telemetry_block(tmp_path):
    probe = subprocess.run(
        ["python3", "-c", "import harness.telemetry"],
        env={**os.environ, "PYTHONPATH": str(_PROJECT_ROOT)},
        capture_output=True,
    )
    if probe.returncode != 0:
        pytest.skip("python3 on PATH is not the harness interpreter")
    proc, body = _run_open_pr(tmp_path, summary_text=_summary_json())
    assert proc.returncode == 0, proc.stderr
    assert body.startswith(_HARDCODED_BODY)
    assert "<!-- deepagents:telemetry -->" in body
    assert "### Run telemetry" in body
    assert "46,142" in body           # tokens, formatted in Python not sh
    assert "6m 52s" in body           # durations, likewise
    assert "not priced" in body       # never a fabricated $0.00


@pytest.mark.skipif(not (_HAS_SH and _HAS_PY3), reason="needs sh and python3")
def test_pr_body_carries_aggregates_only(tmp_path):
    """Invariant 12: a planted secret in a turn record cannot reach the body.
    Two independent reasons -- the record is scrubbed on the way in, and the block
    is built from the summary, which has no free-text field."""
    probe = subprocess.run(
        ["python3", "-c", "import harness.telemetry"],
        env={**os.environ, "PYTHONPATH": str(_PROJECT_ROOT)},
        capture_output=True,
    ...
    if probe.returncode != 0:
        pytest.skip("python3 on PATH is not the harness interpreter")
    proc, body = _run_open_pr(tmp_path, summary_text=_summary_json(topic="sk-abcdefghijklmnopqrstuv"))
    assert proc.returncode == 0, proc.stderr
    assert "sk-abcdefghijklmnopqrstuv" not in body
