...
`priority = 0`, so an unconfigured run uses a local model — set `OLLAMA_HOST` so the container can
reach the host daemon. See `providers/README.md` → "Auto-selection: two gates".
