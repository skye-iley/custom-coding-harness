...
> **Slice H (bwrap fs-tool jail) is core v1 scope, not stretch** (`milestone4.md` §3) — and it is not
> yet built. Invariants 5 (3rd/4th leg), 14 (shell coverage), and 16 (approvable-exception flow) are
> the three that depend on H and are **not yet satisfied**. Everything else below is built and green,
> but the milestone as a whole is not done until H lands and these three close out.
...
5. Floor redundant. **Currently enforced 2 ways independently** — docker mask always emits it, and the resolver drops any negation/allow of it. **The 3rd+4th legs (file backend explicitly refuses a floor path; bwrap never binds it) are REQUIRED v1 scope, not built yet** — `agent.py:_resolve_path` only checks for workspace escape (`pathguard.validate_path`), it has no floor/mask awareness; a floor file simply reads empty because the docker overlay changed the real fs. Those legs land with slice H, which is **core scope, not a stretch layer** (`milestone4.md` §3) — so the "≥3 independent" redundancy this invariant describes is not yet satisfied, and the milestone is not complete while that's true. *(Tested: `TestFloorRedundancy` documents the two working legs and marks the 3rd/4th as the H-blocked remainder.)*
...
16. *(blocked on H, required for v1)* Approvable scope bounded. Operator may approve one-off only for in-bounds, non-floor path. Escape (realpath outside workspace) never approvable. *(Nothing is approvable today — structurally satisfied but not exercised: `hitl.make_path_denied_handler` has no code path that returns `True`. This isn't a permanent v1 shape — it's this way because the "in-bounds but masked" denial type doesn't exist until H makes a masked-read denial explicit. H is required v1 scope (`milestone4.md` §3), so this invariant becomes actually checkable, not just structurally-true-by-vacuity, once H ships.)*
