...
> Stretch **H (bwrap fs-tool jail) is now built and opt-in** (`DEEPAGENTS_JAIL=1`, §11.4): the userns
> gate verified under a vendored narrow seccomp profile, and routing is by **re-exec of the harness
> process**, not per-tool wrappers. Deferred v2 (overlayfs view) not yet built. See
...
| **H** bwrap fs-tool jail | allow-list boundary; route **all** fs tools through the jail; narrow seccomp profile; **carries slice D's deferred approve branch** | feature plan §4.2 · detail §11.4 | **stretch** (opt-in, `DEEPAGENTS_JAIL=1`) | PR6 |
